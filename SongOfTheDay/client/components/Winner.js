...
function Winner() {
  ...
  const findWinningSongs = () => {
    ...
    if (selectedQuestion) {
      ...
      setWinningSongs([{ song: winningSong, user: winningUser }]);
    } else {
      setWinningSongs([]);
    }
  };
    ...
    <div>
      ...
      {winningSongs.length > 0 && (
        <div>
          <h2>Winning Songs for {selectedDate}:</h2>
          <ul>
            {winningSongs.map((winning) => (
              <li key={winning.song.id}>
                {winning.song.song.artist} - {winning.song.song.name}
               <div> {winning.user.username}
               </div>
              </li>
            ))}
          </ul>
